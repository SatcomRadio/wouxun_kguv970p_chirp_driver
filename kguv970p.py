"""Wouxun KG-UV970P radio management module"""
...
_MEM_FORMAT = """
    ...
    #seekto 0x00b0;
    struct {
        u8      bandslice_am_spacing;
        u8      bandslice_144M_spacing;
        u8      bandslice_70cm_spacing;
        u8      bandslice_10m_spacing;
        u8      bandslice_6m_spacing;
        u8      bandslice_350M_spacing;
        u8      bandslice_850M_spacing;
    } band_spacing;
        
    #seekto 0x00c0;
    struct {
        bbcd    bandslice_am_start[2];
        bbcd    bandslice_144M_start[2];
        bbcd    bandslice_70cm_start[2];
        bbcd    bandslice_10m_start[2];
        bbcd    bandslice_6m_start[2];
        bbcd    bandslice_350M_start[2];
        bbcd    bandslice_850M_start[2];
    } band_start;
...
# Support for the Wouxun KG-UV970P radio
...
@directory.register
class KG970PRadio(chirp_common.CloneModeRadio,
    ...
    def _get_tone(self, _mem, mem):
        #  - corrected the Polarity decoding to match implementation
    ...
    def _get_settings(self):
        ...
        extra_grp.append(RadioSettingGroup("lmwrn_grp", "USE AT YOUR OWN RISK"))

        group = RadioSettings(cfg_grp, vfo_grp, fmradio_grp, remote_grp, scan_grp, extra_grp)

        self._generate_config1_settings(cfg_grp)
        self._generate_config2_settings(cfg_grp)
        self._generate_vfo_settings(vfo_grp)
        self._generate_fm_radio_settings(fmradio_grp)
        self._generate_remote_group_settings(remote_grp)
        self._generate_scan_group_settings(scan_grp)
        self._generate_oem_settings(extra_grp)
        self._generate_limit_settings(extra_grp)
        self._generate_slices_settings(extra_grp)
        self._generate_pwr_settings(extra_grp)
        self._generate_modulation_settings(extra_grp)

        return group

    def _generate_config1_settings(self, cfg_grp):
        _settings = self._memobj.settings
    ...
    def _generate_config2_settings(self, cfg_grp):
        _oem = self._memobj.oem
        _settings = self._memobj.settings
        ...
        for i in range(2, 8):
            ...
            if _psw[i] in DTMF_CHARS:
                _code += _psw[i]
        val_psw = RadioSettingValueString(6, 6, _code, False)
        val_psw.set_charset(DTMF_CHARS)
    ...
    def _generate_vfo_settings(self, vfo_grp):
        _settings = self._memobj.settings
        wmmap = WORKMODE_MAP
        narrow1label = "VHF Bandwidth"
        mute1label = "VHF Mute Mode"
        scram1label = "VHF Scrambler"
        narrow2label = "SAT Bandwidth"
        mute2label = "SAT Mute Mode"
        scram2label = "SAT Scrambler"
        ...
        vfo2m_grp = RadioSettingGroup("vfo2m_grp", "VHF Settings")
        vfo300_grp = RadioSettingGroup("vfo300_grp", "SAT Settings")
        ...
        vfoa_grp.append(vfo2m_grp)
        vfoa_grp.append(vfo300_grp)
        ...
        vfoa_grp.append(rs)
        rs = RadioSetting("vfofreq1", "VHF Freq",
                          RadioSettingValueFloat(
                              0, 999.999999, (_freq_decode
                                              (_settings.vfofreq1) /
                                              1000000.0), 0.000001, 6))
        vfo2m_grp.append(rs)
        rs = RadioSetting("vfoofst1", "VHF Offset",
                          RadioSettingValueFloat(
                              0, 999.999999, (_freq_decode
                                              (_settings.vfoofst1) /
                                              1000000.0), 0.000001, 6))
        vfo2m_grp.append(rs)
        rs = RadioSetting("rxtone1", "VHF Rx tone",
                          RadioSettingValueMap(
                              TONE_MAP, _settings.rxtone1))
        vfo2m_grp.append(rs)
        rs = RadioSetting("txtone1", "VHF Tx tone",
                          RadioSettingValueMap(
                              TONE_MAP, _settings.txtone1))
        vfo2m_grp.append(rs)
        rs = RadioSetting("power1", "VHF Power",
                          RadioSettingValueMap(
        ...
        vfo2m_grp.append(rs)
        rs = RadioSetting("shft_dir1", "VHF Shift Direction",
                          RadioSettingValueList(
                              DUPLEX_LIST,
                              current_index=_settings.shft_dir1))
        vfo2m_grp.append(rs)
        rs = RadioSetting("compander1", "VHF Compander",
        ...
        vfo2m_grp.append(rs)
        rs = RadioSetting("am_mode1", "VHF AM Mode",
                          RadioSettingValueBoolean(
                              _settings.am_mode1))
        vfo2m_grp.append(rs)
        # ###########################
        rs = RadioSetting("vfofreq2", "SAT Freq",
                          RadioSettingValueFloat(
                              0, 999.999999, (_freq_decode(
                                  _settings.vfofreq2) /
                                              1000000.0), 0.000001, 6))
        vfo300_grp.append(rs)
        rs = RadioSetting("vfoofst2", "SAT Offset",
                          RadioSettingValueFloat(
                              0, 999.999999, (_freq_decode(
                                  _settings.vfoofst2) /
                                              1000000.0), 0.000001, 6))
        vfo300_grp.append(rs)
        rs = RadioSetting("rxtone2", "SAT Rx tone",
                          RadioSettingValueMap(
                              TONE_MAP, _settings.rxtone2))
        vfo300_grp.append(rs)
        rs = RadioSetting("txtone2", "SAT Tx tone",
                          RadioSettingValueMap(
                              TONE_MAP, _settings.txtone2))
        vfo300_grp.append(rs)
        rs = RadioSetting("power2", "SAT Power",
                          RadioSettingValueMap(
        ...
        vfo300_grp.append(rs)
        rs = RadioSetting("shft_dir2", "SAT Shift Direction",
                          RadioSettingValueList(
                              DUPLEX_LIST,
                              current_index=_settings.shft_dir2))
        vfo300_grp.append(rs)
        rs = RadioSetting("compander2", "SAT Compander",
                          RadioSettingValueBoolean(
        ...
        vfo300_grp.append(rs)
        rs = RadioSetting("am_mode2", "SAT AM Mode",
                          RadioSettingValueBoolean(
                              _settings.am_mode2))
        vfo300_grp.append(rs)
    ...
    def _generate_fm_radio_settings(self, fmradio_grp):
        _settings = self._memobj.settings
    ...
    def _generate_remote_group_settings(self, remote_grp):
        _settings = self._memobj.settings
        ...
        val_ani.set_charset(DTMF_CHARS)
        ...
        val_mcc.set_charset(DTMF_CHARS)
        ...
        val_scc.set_charset(DTMF_CHARS)
    ...
    def _generate_oem_settings(self, extra_grp):
        oem_grp = RadioSettingGroup("oem_grp", "OEM Info")
        extra_grp.append(oem_grp)

        _oem_name = _oem_str_decode(self._memobj.oem.display_name)
        rs = RadioSetting("oem.display_name", "Display Message",
                          RadioSettingValueString(1, 8, _oem_name))
        oem_grp.append(rs)

    def _generate_limit_settings(self, extra_grp):
        ...
        rs = RadioSetting("bandlimits.limit_144M_ChA_rx_start",
                          "VHF VFO A Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_144M_ChA_rx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_144M_ChA_rx_start",
                          "Hardware VHF VFO A Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_144M_ChA_rx_stop",
                          "VHF VFO A Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_144M_ChA_rx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_144M_ChA_rx_stop",
                          "Hardware VHF VFO A Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_144M_ChB_rx_start",
                          "VHF VFO B Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_144M_ChB_rx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_144M_ChB_rx_start",
                          "Hardware VHF VFO B Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_144M_ChB_rx_stop",
                          "VHF VFO B Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_144M_ChB_rx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_144M_ChB_rx_stop",
                          "Hardware VHF VFO B Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_70cm_rx_start",
                          "SAT Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_70cm_rx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_70cm_rx_start",
                          "Hardware SAT Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_70cm_rx_stop",
                          "SAT Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_70cm_rx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_70cm_rx_stop",
                          "Hardware SAT Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        _temp = int(s.hw_bandlimits.limit_6m_rx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_6m_rx_start",
                          "Hardware 50M Rx Lower Limit (MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        _temp = int(s.hw_bandlimits.limit_6m_rx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_6m_rx_stop",
                          "Hardware 50M Rx Upper Limit (+ .9975 MHz)",
                          val)
        rxlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_144M_tx_start",
                          "VHF Tx Lower Limit (MHz)",
                          val)
        txlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_144M_tx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_144M_tx_start",
                          "Hardware VHF Tx Lower Limit (MHz)",
                          val)
        txlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_144M_tx_stop",
                          "VHF Tx Upper Limit (+ .9975 MHz)",
                          val)
        txlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_144M_tx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_144M_tx_stop",
                          "Hardware VHF Tx Upper Limit (+ .9975 MHz)",
                          val)
        txlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_70cm_tx_start",
                          "SAT Tx Lower Limit (MHz)",
                          val)
        txlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_70cm_tx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_70cm_tx_start",
                          "Hardware SAT Tx Lower Limit (MHz)",
                          val)
        txlim_grp.append(rs)
        ...
        rs = RadioSetting("bandlimits.limit_70cm_tx_stop",
                          "SAT tx Upper Limit (+ .9975 MHz)",
                          val)
        txlim_grp.append(rs)

        _temp = int(s.hw_bandlimits.limit_70cm_tx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_70cm_tx_stop",
                          "Hardware SAT Tx Upper Limit (+ .9975 MHz)",
                          val)
        txlim_grp.append(rs)
        ...
        _temp = int(s.hw_bandlimits.limit_6m_tx_start) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_6m_tx_start",
                          "Hardware 50M Tx Lower Limit (MHz)",
                          val)
        txlim_grp.append(rs)
        ...
        _temp = int(s.hw_bandlimits.limit_6m_tx_stop) // 10
        val = RadioSettingValueInteger(0, 999, _temp)
        rs = RadioSetting("hw_bandlimits.limit_6m_tx_stop",
                          "Hardware 50M Tx Upper Limit (+ .9975 MHz)",
                          val)
        txlim_grp.append(rs)

    def _generate_slices_settings(self, extra_grp):
        slice_grp = RadioSettingGroup("slice_grp", "Band slices")
        extra_grp.append(slice_grp)

        val = RadioSettingValueInteger(0, 999, int(self._memobj.band_start.bandslice_am_start) // 10)
        rs = RadioSetting("band_start.bandslice_am_start", "AM band slice start (MHz)", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 255, self._memobj.band_spacing.bandslice_am_spacing)
        rs = RadioSetting("band_spacing.bandslice_am_spacing", "AM band spacing in Mhz", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 999, int(self._memobj.band_start.bandslice_144M_start) // 10)
        rs = RadioSetting("band_start.bandslice_144M_start", "VHF band slice start (MHz)", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 255, self._memobj.band_spacing.bandslice_144M_spacing)
        rs = RadioSetting("band_spacing.bandslice_144M_spacing", "VHF band spacing in Mhz", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 999, int(self._memobj.band_start.bandslice_70cm_start) // 10)
        rs = RadioSetting("band_start.bandslice_70cm_start", "SAT band slice start (MHz)", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 255, self._memobj.band_spacing.bandslice_70cm_spacing)
        rs = RadioSetting("band_spacing.bandslice_70cm_spacing", "SAT band spacing in Mhz", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 999, int(self._memobj.band_start.bandslice_6m_start) // 10)
        rs = RadioSetting("band_start.bandslice_6m_start", "6M band slice start (MHz)", val)
        slice_grp.append(rs)

        val = RadioSettingValueInteger(0, 255, self._memobj.band_spacing.bandslice_6m_spacing)
        rs = RadioSetting("band_spacing.bandslice_6m_spacing", "6M band spacing in Mhz", val)
        slice_grp.append(rs)

    def _generate_pwr_settings(self, extra_grp):
        pwr_grp = RadioSettingGroup("pwr_grp", "Power levels")
        extra_grp.append(pwr_grp)

        vhf_pwr_grp = RadioSettingGroup("vhf_pwr_grp", "2M power levels")
        pwr_grp.append(vhf_pwr_grp)
        sat_pwr_grp = RadioSettingGroup("sat_pwr_grp", "SAT power levels")
        pwr_grp.append(sat_pwr_grp)
        hf_pwr_grp = RadioSettingGroup("hf_pwr_grp", "50M power levels")
        pwr_grp.append(hf_pwr_grp)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.high_144_pwrlvl.level[i])
            rs = RadioSetting("high_144_pwrlvl.level_" + str(i),
                              "High PWR Slice: " + str(i + 1), val)
            vhf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.med1_144_pwrlvl.level[i])
            rs = RadioSetting("med1_144_pwrlvl.level_" + str(i),
                              "Med1 PWR Slice: " + str(i + 1), val)
            vhf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.med2_144_pwrlvl.level[i])
            rs = RadioSetting("med2_144_pwrlvl.level_" + str(i),
                              "Med2 PWR Slice: " + str(i + 1), val)
            vhf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.low_144_pwrlvl.level[i])
            rs = RadioSetting("low_144_pwrlvl.level_" + str(i),
                              "Low PWR Slice: " + str(i + 1), val)
            vhf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.high_70cm_pwrlvl.level[i])
            rs = RadioSetting("high_70cm_pwrlvl.level_" + str(i),
                              "High PWR Slice: " + str(i + 1), val)
            sat_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.med1_70cm_pwrlvl.level[i])
            rs = RadioSetting("med1_70cm_pwrlvl.level_" + str(i),
                              "Med1 PWR Slice: " + str(i + 1), val)
            sat_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.med2_70cm_pwrlvl.level[i])
            rs = RadioSetting("med2_70cm_pwrlvl.level_" + str(i),
                              "Med2 PWR Slice: " + str(i + 1), val)
            sat_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.low_70cm_pwrlvl.level[i])
            rs = RadioSetting("low_70cm_pwrlvl.level_" + str(i),
                              "Low PWR Slice: " + str(i + 1), val)
            sat_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.high_6m_pwrlvl.level[i])
            rs = RadioSetting("high_6m_pwrlvl.level_" + str(i),
                              "High PWR Slice: " + str(i + 1), val)
            hf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.med1_6m_pwrlvl.level[i])
            rs = RadioSetting("med1_6m_pwrlvl.level_" + str(i),
                              "Med1 PWR Slice: " + str(i + 1), val)
            hf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.med2_6m_pwrlvl.level[i])
            rs = RadioSetting("med2_6m_pwrlvl.level_" + str(i),
                              "Med2 PWR Slice: " + str(i + 1), val)
            hf_pwr_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.low_6m_pwrlvl.level[i])
            rs = RadioSetting("low_6m_pwrlvl.level_" + str(i),
                              "Low PWR Slice: " + str(i + 1), val)
            hf_pwr_grp.append(rs)

    def _generate_modulation_settings(self, extra_grp):
        mod_grp = RadioSettingGroup("modulation_grp", "Modulation")
        extra_grp.append(mod_grp)

        vhf_mod_grp = RadioSettingGroup("vhf_mod_grp", "2M modulation")
        mod_grp.append(vhf_mod_grp)
        sat_mod_grp = RadioSettingGroup("sat_mod_grp", "SAT modulation")
        mod_grp.append(sat_mod_grp)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.modulation_144.level[i])
            rs = RadioSetting("modulation_144.level_" + str(i), "Modulation Slice: " + str(i+1), val)
            vhf_mod_grp.append(rs)

        for i in range(0, 16):
            val = RadioSettingValueInteger(0, 255, self._memobj.modulation_70cm.level[i])
            rs = RadioSetting("modulation_70cm.level_" + str(i), "Modulation Slice: " + str(i+1), val)
            sat_mod_grp.append(rs)
                ...
                try:
                    ...
                    if element.has_apply_callback():
                        ...
                        if self._is_freq(element):
                            ...
                            # expected values
                            setattr(obj, setting, int(element.values()[0]._current * 100.0))

                        elif self._is_limit(element):
                            setattr(obj, setting, int(element[0].get_value()) * 10)

                        elif self._is_band_start(element):
                            setattr(obj, setting, int(element[0].get_value()) * 10)

                        elif self._is_modulation(element):
                            val = int(element[0].get_value())
                            main_setting = element.get_name().split('.', 1)[0]
                            index = int(element.get_name().split('.', 1)[1].split('_')[-1])
                            self._memobj.__dict__["_generators"][main_setting]["level"][index] = val

                        elif self._is_power(element):
                            val = int(element[0].get_value())
                            main_setting = element.get_name().split('.', 1)[0]
                            index = int(element.get_name().split('.', 1)[1].split('_')[-1])
                            self._memobj.__dict__["_generators"][main_setting]["level"][index] = val
    ...
    def _is_band_start(self, element):
        return "band_start" in element.get_name()

    def _is_power(self, element):
        return "_pwrlvl" in element.get_name()

    def _is_modulation(self, element):
        return "modulation_" in element.get_name()
